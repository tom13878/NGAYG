...
seed_prev <-  read_dta(file.path(dataPath, "Post Planting Wave 2/Agriculture/sect11e_plantingw2.dta")) %>%
  dplyr::transmute(hhid, plotid, seed_typ=s11eq5, seed_q=s11eq6a, seed_unit=s11eq6b) %>%
  filter(seed_typ %in% c(1080, 1081, 1082, 1083))

seed_free <-  read_dta(file.path(dataPath, "Post Planting Wave 2/Agriculture/sect11e_plantingw2.dta")) %>%
  dplyr::transmute(hhid, plotid, seed_typ=s11eq9, seed_q=s11eq10a, seed_unit=s11eq10b) %>%
  filter(seed_typ %in% c(1080, 1081, 1082, 1083)) 
  
seed1 <-  read_dta(file.path(dataPath, "Post Planting Wave 2/Agriculture/sect11e_plantingw2.dta")) %>%
  dplyr::transmute(hhid, plotid, seed_typ=s11eq17, seed_q=s11eq18a, seed_unit=s11eq18b) %>%
  filter(seed_typ %in% c(1080, 1081, 1082, 1083))

seed2 <-  read_dta(file.path(dataPath, "Post Planting Wave 2/Agriculture/sect11e_plantingw2.dta")) %>%
  dplyr::transmute(hhid, plotid, seed_typ=s11eq29, seed_q=s11eq30a, seed_unit=s11eq30b) %>%
  filter(seed_typ %in% c(1080, 1081, 1082, 1083)) 

seed_all <- rbind(seed_prev, seed_free, seed1, seed2) %>% 
  rename(qty_unit = seed_unit) %>%
  mutate(qty_unit = as.character(qty_unit)) %>%
  left_join(., conv_seed)  %>%
...
comseed <-  read_dta(file.path(dataPath, "Post Planting Wave 2/Agriculture/sect11e_plantingw2.dta")) %>%
  dplyr::select(hhid, plotid, seed=s11eq14, seed_typ=s11eq17) %>%
  filter(seed_typ %in% c(1080, 1081, 1082, 1083)) %>%
  select(-seed_typ) %>% 
  unique # remove duplicates, apparently some plots have multiple entries for maize.

seed <- left_join(seed_all, comseed) %>%
...
se <- read_dta(file.path(dataPath, "Post Planting Wave 2/Household/sect1_plantingw2.dta")) %>%
  dplyr::transmute(hhid, indiv, sex=as_factor(s1q2), status=as_factor(s1q3), age=s1q6) 
...
plotrights$plot_rightboth_others <- ifelse(plotrights$plot_rightboth_others %in% 2, 0, plotrights$plot_rightboth_others)
...
################################
# note number of extension data recorded is lower
...
credit <- read_dta(file.path(dataPath, "Post Planting Wave 2/Household/sect4a_plantingw2.dta")) %>%
  dplyr::select(hhid,  bank_account_own=s4aq1, bank_account_others4use = s4aq7, coop_sav=s4aq8, borrow_dummy=s4aq11 )
...
communitysum_ph <- group_by(community_ph, EAID, DISNAME) %>%
  summarise(com_type_agricoop=max(com_type_agricoop, na.rm=T), com_type_creditcoop=max(com_type_creditcoop, na.rm=T))
...
NGA2012 <- left_join(NGA2012, seed); rm(seed) # ONE duplicate CHECK
NGA2012 <- left_join(NGA2012, plotrights); rm(plotrights)

# Set seed to 0 as only plots with commercial seed have been linked
NGA2012$seed <- ifelse(is.na(NGA2012$seed), 0, NGA2012$seed)
...
NGA2012 <- left_join(NGA2012, ICT_ph); rm(ICT_ph)
NGA2012 <- left_join(NGA2012, credit_ph); rm(credit_ph)
NGA2012 <- left_join(NGA2012, credit_pp); rm(credit_pp)
NGA2012 <- left_join(NGA2012, ext_ph); rm(ext_ph)
NGA2012 <- left_join(NGA2012, ext_pp); rm(ext_pp)

# -------------------------------------
# community level joins
NGA2012 <- left_join(NGA2012, comm_ph); rm(comm_ph)
NGA2012 <- left_join(NGA2012, infra_ph); rm(infra_ph)
...
# Remove all labels
NGA2012 <- remove_all_labels(NGA2012)
