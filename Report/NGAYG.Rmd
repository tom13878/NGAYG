...
source(file.path(root ,"Code/NGA_tbl.R"))
...
Figure X depicts the three yield levels that can be derived on the basis of the economic, technical efficiency and technology constraints discussed above: 
...
```{r Fig_YGF}
knitr::include_graphics("../FigTab/Fig_YG_fw.png")
...
library(readr)
library(knitr)
Tbl_YG <- read_csv(file.path(root, "FigTab/Tbl_YG.csv"))
kable(Tbl_YG, caption = "Table 1: Yield gaps")
```
...
#and also ignores the possibility that farmers might lose crop area between planting and harvest (e.g., partial crop failure). Drawing
```
