...
# Introduction
Yield gap is a powerful concept to illustrate the potential to increase future crop yield. It is defined as the difference between potential yield and actual yield observed at the plot, farm or regional level. Despite its abundant use, the yield gap can be defined and measured in a number of ways, which has resulted in lack of consistency in yield gap analysis in the literature [@Lobell2009]. Furthermore, in a recent review of the use of yield gap analysis in key policy papers @Sumberg noted that _"there is a tension between the notion of yield gap as developed in crop ecology (although even here there is no single or consistent usage) and micro-economic studies"_ [p. 510]. 

The aim of this paper is to address some of these criticisms by integrating micro-economic and agronomic yield gap approaches into one single framework. It extends the work of @Fisher2015, who recently reviewed definitions of crop yield and yield gaps and builds on the work of Van Dijk et al. [@VanDijk] and Silva et al. [@Silva], who combined agroniomic and economic approaches to yield gap analysis. 

We critically review some of the most comon yield gap definitions and higlight a number of inconsistencies. We analyse the definition of actual yield... [ADD]. In partical we demonstrate that the definition and use of 'attainable yield' is fraught with difficulties and in practice has been used to define several conceptually different production levels. 

We also critically review the use of the term 'exploitable yield level [@Cassman2003, @VanIttersum2013], sometimes referred to as 'economic yield level'. Exploitable yield is used to capture the part of the yield gap that will not be closed because of economic constraints. It is generaly equated to be 75-85% of potential yield [@Cassman1999; @Cassman2003, @VanIttersum2013]. As pointed out by @Fischer in a recent review on definitions of crop yield and yield gaps, these numbers are based on "general experience" [p.11] and mainly represent to situations _"where there is no other competition for the farmers’ resources, and world prices and reasonable transport costs operate"_ [p.11]. He also points out that in situtations Where this does not occur (e.g. Sub-Saharan Africa) characterised by poor infrastructure and weak institutions, the exploitable yield gap is expected to be much higher. Despite its weak underpinnings the 75-80% 'rule of thumb' is applied frequently as a feasible target in policies to assess potential to increase future crop production. [@vanOort2014, @Aramburo2015], which can be potentialy misleading, in particular for Sub-Saharan African countries.

We argue that the definition of (true) economic yield should be rooted in neoclassical economic theory, the dominant paradigm in econimics, and be estimatd using information on the prices of inputs and outputs. Analogue to arguments in crop ecology, which stress the localised nature of agroclimatic conditions, we argue that economic yield levels are location specific. It is well-known that in many developing countries (subnational) trade is limited due to poor infrastructure resulting in isolated markets and differentiated market prices [@WorldBank].

Finally, to conceptually clarify the different yield levels and yield gaps we introduce two new yield levels that build on economic efficiency estimation and are now regularly used to estimate yield gaps [REF]. 
